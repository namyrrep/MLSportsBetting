...
function TrendChart({ weekly }) {
  ...
  const maxAccuracy = Math.max(...weekly.map(w => w.accuracy || 0));
  const minAccuracy = Math.min(...weekly.map(w => w.accuracy || 0));
    ...
    <div className="trend-chart">
      ...
      <div className="chart-container">
        <svg className="chart-svg" viewBox="0 0 600 200" preserveAspectRatio="none">
          ...
          {/* Grid lines */}
          <line x1="0" y1="50" x2="600" y2="50" stroke="rgba(148, 163, 184, 0.1)" strokeWidth="1" />
          <line x1="0" y1="100" x2="600" y2="100" stroke="rgba(148, 163, 184, 0.15)" strokeWidth="1" />
          <line x1="0" y1="150" x2="600" y2="150" stroke="rgba(148, 163, 184, 0.1)" strokeWidth="1" />
          
          {/* Area fill */}
          <path
            d={`M 0 200 ${weekly.map((w, i) => {
              const x = (i / (weekly.length - 1 || 1)) * 600;
              const y = 200 - ((w.accuracy - minAccuracy + 0.1) / (range + 0.2)) * 180;
              return `L ${x} ${y}`;
            }).join(' ')} L 600 200 Z`}
            ...
            return (
              <g key={i}>
                <circle
                  ...
                  strokeWidth="2"
                />
                <title>Week {w.week}: {(w.accuracy * 100).toFixed(1)}%</title>
              </g>
            );
          })}
        </svg>
        <div className="chart-labels">
          {weekly.map((w, i) => (
            <span key={i} className="chart-label">W{w.week}</span>
          ))}
        </div>
      </div>
    </div>
...
export default function Dashboard() {
  ...
  const handleRetry = () => {
    setSelectedWeek(prev => prev); // Trigger re-fetch
  };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="week-season-selector">
